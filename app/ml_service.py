    ...
    try:
        with mlflow.start_run(run_name=f"{instrument_uid}_{interval}") as run:
            ...
            mlflow.pyfunc.log_model(
                ...
                registered_model_name=f"Price_Predictor_{instrument_uid}_{interval}", # Имя, под которым модель появится в Model Registry
            )

            mlflow.log_artifacts(str(path), artifact_path=f"{instrument_uid}_{interval}")

            # Фиксируем метрики успешного обучения
            ...
            # MAPE каждой модели из leaderboard
            for _, row in leaderboard.iterrows():
                if pd.notnull(row.get("score_val")):
